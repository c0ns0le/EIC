...
$DestinationFolder = "c:\deploy\sharepoint\2013\sharepoint\prerequisiteinstallerfiles"

if (!(test-path $DestinationFolder)) { $DestinationFolder = split-path -parent $MyInvocation.MyCommand.Definition }


#clear

while (!(test-connection "download.microsoft.com" -quiet))
	{
	write-host "Can't reach download.microsoft.com to download SP2013 Prerequisites."
	$x = read-host "Enter 'x' to skip, any other key to try again."
	if ($x -eq "x") {break} 
	} 
...
function DownloadSP2013Files()
{
	...
	ForEach ($FileUrl in $FilesUrlList)
	{
	    ...
	    if ($FileUrl -match "1CAA41C7-88B9-42D6-9E11-3C655656DAB1") {$DestinationFileName = $DestinationFileName -replace ".exe","56.exe"}
		Try
		{
